...
newdata <- subset(newdata, select = c(SalePrice, LotArea, HouseAge, TotalFloorSF, SaleCondition, PavedDrive, GarageArea, GarageFinish, FireplaceQu, KitchenQual, TotRmsAbvGrd, OverallQual, YearBuilt, BsmtQual, TotalBsmtSF, FirstFlrSF, GrLivArea, FullBath, GarageCars, SecondFlrSF, CentralAir))

newdatatable <- table(lapply(newdataintegers, mean))

newdataintegerssum <- summary(newdataintegers, na.rm = TRUE)
newdataintegerssum

newdatafactorssum <- summary(newdatafactors)
newdatafactorssum
...
ggplot(newdata, aes(x=SalePrice)) + 
  geom_histogram(color="blue", binwidth= 10000) +
...
ggplot(newdata, aes(x= CentralAir, y=SalePrice)) + 
  geom_boxplot(fill="blue") +
  labs(title="Distribution of Sale Price") +
  theme(plot.title=element_text(lineheight=0.8, face="bold", hjust=0.5))

ggplot(newdata, aes(x=Functional, y=SalePrice)) + 
